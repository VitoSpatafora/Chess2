...
import chesscore # This is your compiled C++ module
import os
...
PIECE_CHAR_TO_CPP_ENUM_INDEX = {
    'P': chesscore.W_PAWN, 'N': chesscore.W_KNIGHT,
    'B': chesscore.W_BISHOP, 'R': chesscore.W_ROOK,
    'Q': chesscore.W_QUEEN, 'K': chesscore.W_KING,
    'p': chesscore.B_PAWN, 'n': chesscore.B_KNIGHT,
    'b': chesscore.B_BISHOP, 'r': chesscore.B_ROOK,
    'q': chesscore.B_QUEEN, 'k': chesscore.B_KING,
}

CPP_PROMO_TYPE_TO_CHAR = {
    getattr(chesscore.PromoPieceType, "PROMO_TYPE_N").value: 'N',
    getattr(chesscore.PromoPieceType, "PROMO_TYPE_B").value: 'B',
    getattr(chesscore.PromoPieceType, "PROMO_TYPE_R").value: 'R',
    getattr(chesscore.PromoPieceType, "PROMO_TYPE_Q").value: 'Q'
}
...
def _parse_fen_to_cpp_position(fen_string):
    """
    Parses a FEN string and populates a chesscore.Position object.
    ...
    try:
        ...
        cpp_castling = 0
        if hasattr(chesscore.Position, "WK_CASTLE_MASK"):
            if 'K' in js_castling_rights_fen_str: cpp_castling |= chesscore.Position.WK_CASTLE_MASK
            if 'Q' in js_castling_rights_fen_str: cpp_castling |= chesscore.Position.WQ_CASTLE_MASK
            if 'k' in js_castling_rights_fen_str: cpp_castling |= chesscore.Position.BK_CASTLE_MASK
            if 'q' in js_castling_rights_fen_str: cpp_castling |= chesscore.Position.BQ_CASTLE_MASK
        else:
...
def decode_cpp_move_to_js(cpp_move_int, player_who_moved_color):
    ...
    promo_type_none_val = getattr(chesscore.PromoPieceType, "PROMO_TYPE_NONE").value
...
try:
    ...
    print(f"CRITICAL ERROR: Failed to initialize chesscore.Engine: {e}")
    traceback.print_exc()
...
def get_ai_move_route():
    ...
    try:
        ...
        if best_move_cpp_int == 0: # No legal moves
            ...
            outcome_map = {
                chesscore.Outcome.CHECKMATE: f'AI ({ai_player_color}) is Checkmated.',
                chesscore.Outcome.STALEMATE: f'AI ({ai_player_color}) is Stalemated.',
                chesscore.Outcome.DRAW_THREEFOLD_REPETITION: f'Draw by Threefold Repetition for AI ({ai_player_color}).',
                chesscore.Outcome.DRAW_FIFTY_MOVE: f'Draw by Fifty Move Rule for AI ({ai_player_color}).',
                chesscore.Outcome.ONGOING: f'AI ({ai_player_color}) has no legal moves (Ongoing but no moves?).' # Should ideally be caught by checkmate/stalemate
            }
...
if __name__ == '__main__':
    ...
    if run_perft_test:
        print("\n--- Running Perft Test ---")
        perft_engine = chesscore.Engine(1) # Depth for engine doesn't matter for perft method itself
        ...
        if cpp_pos_for_perft:
            ...
            try:
                ...
                print(f"AttributeError during perft: {ae}. Is 'perft' method bound correctly in chesscore for the Engine class?")
                traceback.print_exc()
            except Exception as e:
